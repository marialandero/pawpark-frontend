...
class _FormQuedadaScreenState extends State<FormQuedadaScreen> {
  ...
  // Variable para almacenar los perros seleccionados (puede quedar vacía)
  List<int> _mascotasSeleccionadas = [];
  ...
  Future<void> _submit() async {
    ...
    final payload = {
      ...
      // Formato ISO que entiende LocalDateTime.parse()
      "mascotasIds": _mascotasSeleccionadas,
      // Se envía la lista (vacía o con IDs)
    };
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (v) =>
                          v!.isEmpty ? "El lugar es obligatorio" : null,
                    ),
                    SizedBox(height: 20),
                    ...
                    SizedBox(height: 30),
                    Text(
                      "¿Qué perros te acompañan? (Opcional)",
                      style: TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                        color: color.primary,
                    ...
                    if (userProvider.usuario != null &&
                        userProvider.usuario!.mascotas.isNotEmpty)
                      ...userProvider.usuario!.mascotas
                          .map(
                            (m) => CheckboxListTile(
                              title: Text(m.nombre),
                              secondary: const Icon(Icons.pets),
                              value: _mascotasSeleccionadas.contains(m.id),
                              onChanged: (bool? seleccionado) {
                                setState(() {
                                  if (seleccionado == true) {
                                    _mascotasSeleccionadas.add(m.id!);
                                  } else {
                                    _mascotasSeleccionadas.remove(m.id);
                                  }
                                });
                              },
                      ...
                      Text(
                        "No tienes mascotas registradas para seleccionar.",
                        style: TextStyle(
                          color: Colors.grey,
